...
- [~] M7.1 Reference codec `compression/jxs` (C++20, bit-exact): [x] codestream parser + geometry (Annex A/B, `jxs_info`), [x] entropy decoder + encoder (Annex C/D; re-encodes the ISO encoder's precincts byte-exactly), [x] DWT/RCT/Star-Tetrix/NLT scaling (E–G, forward + inverse), [x] `jxs_decode` sample-exact vs the ISO decoder on 16 CAM4 streams and vs the 7 ISO 21122-4 Bayer conformance vectors (`compression/tools/verify_decoder.sh`, 2026-09-22), [ ] encoder with CBR `(Q,R)` search + `jxs_encode`, [ ] lossless round trips through the codestream
