# Tauro DA322 one-camera image — timing constraints (DA326 reference fpga_cpnx.sdc, one receiver).
...
create_clock -name {MIPI_CAM_CLK_P} -period 1.33333333333333 [get_ports MIPI_CAM_CLK_P]
set_clock_groups -group [get_clocks {apb_clk hif_clk sif_clk}] -group [get_clocks {ETH_REFCLK_P usr_clk}] -group [get_clocks u_pcs_clk_hf_clk_out_o] -group [get_clocks {clk_byte_hs_0}] -group [get_clocks {eclkout_0}] -group [get_clocks {MIPI_CAM_CLK_P}] -group [get_clocks {ptp_clk}] -asynchronous
