// DA322 variant of the reference camera receiver (holoscan-sensor-bridge 2.7.0
// fpga/nv_mipi_ref_design/mipi_cpnx_ref_design/rtl/mipi_cam_rcvr/mipi_cam_rcvr.sv): adds the per-camera
// CSI data-type filter and detected-type latch that Tauro's DA322 registers expose (MIPI_DT_CTRL/STAT).
...
#(
  ...
  // APB Interface
    ...
    input                     i_axis_tready,
    // DA322 CSI data-type filter (Tauro MIPI_DT_CTRL semantics): 0 = forward every image line,
    // otherwise forward only long packets whose data type matches. o_dt_seen latches the last
    // long-packet data type other than 0x00/0x01 (MIPI_DT_STAT); i_dt_seen_clear resets it.
    input      [7:0]          i_dt_filter,
    input                     i_dt_seen_clear,
    output reg [7:0]          o_dt_seen,
...
//------------------------------------------------------------------------------
  ...
  // i_dt_seen_clear is quasi-static (held by software for the duration of ST_CLEAR); sampled here.
  always @(posedge clk_byte_hs_o or posedge clk_byte_hs_rst) begin
    if (clk_byte_hs_rst) begin
      ...
      if (i_dt_seen_clear) o_dt_seen <= '0;
      ...
      if (({payload_en_o,r_payload_en}==3'b100) || (sp_en_o)) begin
        ...
        wc_r             <= wc;
        dt_r             <= dt;
        dt_pass_r        <= (i_dt_filter == 8'h00) || (dt == i_dt_filter[5:0]);
        if (dt > 6'h01) o_dt_seen <= {2'b00, dt};
        vc_r             <= vc_idx;
        byte_incr_r      <= '0;
        ...
        mipi_axis_tvalid   <= is_frame_vc[vc_r] && dt_pass_r;
        ...
        mipi_axis_tdata    <= '0;
        mipi_axis_tvalid   <= is_frame_vc[vc_r] && dt_pass_r; // Line Padding
        mipi_axis_tlast    <= last_cycle;
        byte_cnt           <= byte_cnt + byte_incr_r;
        mipi_axis_tuser[1] <= last_cycle;
        mipi_axis_tuser[W_AXIS_USER-1:2] <= vc_r;
