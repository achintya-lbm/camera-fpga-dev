...
| `BIN2_RAW12` + `hmax: 341` (2026-09-23, RoCE, 30 s) | 1776×1778 RAW12 | 891 Mbps | 341 / 3630 | 59.98 | **59.96** | 2.273 Gbps | 1796 | 0 / 0 | 0 / 1796 |
...
- With the FRAMOS table values every 3556-line readout (full frame 10-bit, full frame 12-bit, 2×2 binned)
  tops out at the same 32.6 fps: the line time is 628 clocks of 74.25 MHz at every lane rate the DA322's
  1.5 Gbps/lane D-PHY accepts, and binning still scans all rows (`DESIGN.md` §4.1). Binning cuts the
  link load 4× and, with HMAX forced to 341 (`hmax: 341` in the rig config), also reaches 60 fps with
  correct images (2026-09-23). Shorter lines (HMAX ≤ 280) make the sensor emit CRC-clean but completely
  flat frames — check image content, not just CRCs, when pushing timing.
