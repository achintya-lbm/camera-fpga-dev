...
|---|---|---|---|
| `holoscan_sdk` | Holoscan SDK 3.9.0 | core (`libholoscan_core.so`), ping/bayer_demosaic/format_converter/holoviz operators, holoviz module (`libholoscan_viz.so`), UCX GXF extension | hand-written BUILD; build-system-only patches in `holoscan/patches/` |
| `gxf` | GXF 5.1.0 (`5.1.0_20251114_0652b7b15_holoscan-sdk-cu13`) | Graph Execution Framework runtime under Holoscan | **binary** package from NVIDIA's Artifactory (the only one) |
...
| `magic_enum` | 0.9.3 | GXF/Holoscan pin (top-level header layout) | header-only |
